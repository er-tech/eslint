...
const getTSFunctionComment = function (astNode) {
  ...
  // @ts-expect-error -- For `ClassProperty`.
    ...
    /* v8 ignore next 3 */
      ...
      /* v8 ignore next 3 */
        if (!greatGreatGrandparent?.parent) {
          return astNode
        }
 ...
 */
const findJSDocumentComment = (astNode, sourceCode, settings, options = {}) => {
  const { nonJSDoc } = options
 ...
 * @public
 */
const getJSDocumentComment = function (sourceCode, node, settings, options = {}) {
  const reducedNode = getReducedASTNode(node, sourceCode, settings)
  const comment = findJSDocumentComment(reducedNode, sourceCode, settings)

  if (!comment &&
    options.checkOverloads &&
    (
    ...
    const index = programNode.body.indexOf(childNode)
    ...
    if (
      // @ts-expect-error Should be ok
      (previousSibling?.type === 'TSDeclareFunction' &&
        // @ts-expect-error Should be ok
        functionName === previousSibling.id.name) ||
      (previousSibling?.type === 'ExportNamedDeclaration' &&
        // @ts-expect-error Should be ok
        previousSibling.declaration?.type === 'TSDeclareFunction' &&
        // @ts-expect-error Should be ok
        previousSibling.declaration?.id?.name === functionName)
    ) {
      // @ts-expect-error Should be ok
      return getJSDocumentComment(sourceCode, previousSibling, settings, options)
    }
  }
...
const getNonJsdocComment = function (sourceCode, node, settings) {
  ...
  return findJSDocumentComment(reducedNode, sourceCode, settings, {
    nonJSDoc: true,
  })
}
...
export {
  getReducedASTNode, getJSDocumentComment as getJSDocComment, getNonJsdocComment,
  getDecorator, findJSDocumentComment as findJSDocComment, getFollowingComment,
}

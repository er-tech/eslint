...
module.exports = {
  ...
  create (context) {
    ...
    function isDefaultExport (node) {
      return (defaultExport && node.id?.name && defaultExport.value === node.id.name)
    }
    ...
    function checkIfNodeHasTSDocComment (node, typeName, name) {
      const comment = getJSDocComment(context.getSourceCode(node), node, { minLines: 0, maxLines: 1 })
      if (!comment) {
        context.report({
          node,
          messageId: 'no-tsdoc',
